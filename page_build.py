...
function renderPills(){
  ...
  row.querySelectorAll('button[data-cat]').forEach(b=>b.onclick=()=>{
    ...
    if(v==='전체'){V.cats=[];V.tag=null;clearText()}
...
function renderConditions(){
  ...
  el.innerHTML='<span class="clabel">선택한 조건</span>'+
    list.map(function(c){
      ...
      return '<button type="button" class="cbtn" data-cond-off="'+esc(key)+'" '+
        'title="이 조건을 풉니다">'+(c[0]==='term'?'키워드 ':'')+esc(c[1])+
...
function dropCond(key){
  ...
  if(i>=0){if(kind==='cat')V.cats.splice(i,1);else V.terms.splice(i,1)}
