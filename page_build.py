...
.srcrow{display:flex;align-items:center;gap:9px;padding:4px 0;border-bottom:1px solid var(--line)}
.srcrow:last-of-type{border-bottom:0}
.srcrow.off .srcname{color:var(--muted);text-decoration:line-through}
.srcname{flex:1 1 auto;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.srccount{color:var(--muted);font-size:11px}
...
// A switched-off source is dropped by the server on every read path, so there is no filter to apply
// here: this box is the switch itself. The list of sources comes from the collection status, and the
// switches come from the status answer, which carries them only for a session - the anonymous
// response deliberately keeps source detail out.
// The list comes from the status answer, not from the feed: the feed only carries the source map
// after a collection cycle has run, so a panel built from it would sit empty on a fresh start.
...
function renderSourcePanel(){
  const el=document.querySelector('#srcpanel');if(!el)return;
  if(OFF===null){el.innerHTML='<div class="note">목록을 불러오지 못했습니다</div>';return}
  const counts={};
  const src=(SRC_STATE&&Object.keys(SRC_STATE).length)?SRC_STATE:(DATA.sources||{});
  Object.entries(src).forEach(([name,st])=>{counts[name]=(st&&st.count)||0});
  const names=Object.keys(counts).sort((a,b)=>counts[b]-counts[a]||a.localeCompare(b));
  if(!names.length){el.innerHTML='<div class="note">아직 수집 기록이 없습니다</div>';return}
  el.innerHTML=names.map(n=>{
    const off=OFF.indexOf(n)>=0;
    return '<div class="srcrow'+(off?' off':'')+'"><button type="button" class="sw" role="switch"'
      +' aria-checked="'+(off?'false':'true')+'" data-source="'+esc(n)+'" title="'
      +(off?'공개로 되돌리기':'배포 페이지에서 감추기')+'"><span class="knob"></span></button>'
      +'<span class="srcname">'+esc(n)+'</span><span class="srccount">'+counts[n]+'</span></div>'}).join('')
    +'<div class="note" id="srcnote">'+(OFF.length?'숨김 '+OFF.length+'개 · ':'')
    +'감춘 소스는 배포 페이지에서 빠집니다</div>';
  el.querySelectorAll('.sw').forEach(b=>b.onclick=()=>switchSource(b.dataset.source));
}
async function switchSource(name){
  const off=OFF.indexOf(name)>=0, note=()=>document.querySelector('#srcnote');
  if(note())note().textContent='저장 중…';
  try{
    const d=await writeJSON('/api/source',{source:name,hidden:!off});
    OFF=d.hidden_sources||OFF;
    renderSourcePanel();
    if(note())note().textContent=name+(off?' 공개로 되돌렸습니다':' 을 감췄습니다');
    fetchFeed();
  }catch(e){
    renderSourcePanel();
    if(note())note().textContent='저장하지 못했습니다 — '+e.message;
  }
}
// <<< operator-only
...
def render(*, public: bool, datadir: str, want_thai: bool, icon_prefix: str, admin: bool,
    ...
    side = ('' if not admin else
            ...
            '<div class="box"><h2>소스 공개</h2><div id="srcpanel"><div class="note">불러오는 중</div></div></div>'
