...
문서의 언어 선언(`<html lang>`)은 기본 `en`이고, 공개 태국 페이지(`docs/thai/`)만 `th`로 덮어쓴다.

```
index.html            en   로컬
docs/index.html       en   경제 소식
docs/thai/index.html  th   태국 소식
```

수집되는 제목은 대부분 영어이고(경제 소식 89.6% 라틴 문자), 태국 탭은 영어 54%·태국어 37%·한국어 9%다. 화면의 탭·버튼 같은 한국어 UI보다 **기사 제목의 언어 쪽에 선언을 맞췄다.**

번역 제안은 브라우저가 기기 안에서 판단하므로, 페이지가 어느 언어로 보이는지에 따라 번역 메뉴가 뜨기도 하고 안 뜨기도 한다. 선언을 바꿔도 **번역 제안이 반드시 뜬다고 보장할 수는 없다** — 특히 이 페이지는 기사 목록을 스크립트가 나중에 그려서, 처음 파싱하는 시점에는 제목이 아직 없다.

부작용: 한국어 UI 문구를 화면독기가 영어 규칙으로 읽게 된다. 그 부분까지 정확히 하려면 한국어 UI 요소에 `lang="ko"`를 개별로 붙여야 한다.
