...
페이지를 받자마자 보이는 글자는 탭·버튼 같은 한국어 문구뿐이고, 기사 제목은 스크립트가 JSON을 받아 나중에 그린다. 브라우저가 **페이지 언어를 판단하는 시점**에는 기사 텍스트가 아직 없다. 그래서 사파리는 이 페이지를 한국어로 보고, 한국어 기기인데도 **영어로 번역**을 제안한다. 기사 원문 사이트에서 한국어 번역이 뜨는 건 그쪽은 영어 본문이 HTML에 처음부터 들어 있기 때문이다.

그래서 최신 제목 25건을 페이지 HTML에 함께 넣는다(`seed_feed`). 스크립트가 데이터를 받아오면 그 내용으로 대체한다.

```
파싱 시점 화면 텍스트    경제 소식  한국어 54.7% → 20.1% (라틴 51.7%)
                        태국 소식  한국어 9.2%대 → 5.2% (태국어 52.8%)
