...
랜딩은 `landing.py` 에 따로 있다. 대시보드와 공유하는 것이 없어서 한 모듈에 섞어 두면 서로를 건드리게 된다. 살아있는 부분은 최근 뉴스 6건과 히어로의 수치뿐이며, 둘 다 대시보드와 같은 `/api/news` 를 읽는다 — 오래된 숫자가 남지 않고 운영 화면에 있는 것도 드러나지 않는다.
