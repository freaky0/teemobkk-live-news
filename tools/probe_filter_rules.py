...
def run_py(code: str, timeout: int = 120):
    """운영 서버의 코어를 불러 실행한다. 코드는 표준입력으로 넘긴다.

    코드에 한국어가 들어가므로 셸 인용으로 넘기면 안 된다. 인코딩은 URL용이지 셸용이 아니다.
    """
    return ssh("cd %s && PYTHONIOENCODING=utf-8 python3 -" % DIR, input=code, timeout=timeout)
...
def cleanup():
    """프로브가 만든 것만 지운다. 운영자가 숨긴 기사와 그 밖의 기사는 건드리지 않는다."""
    ssh("cd %s && python3 -c \"import live_news_dashboard as c; "
        ...
        "con.execute(\\\"DELETE FROM filter_rules WHERE pattern = ? OR origin = 'learned'\\\", ('%s',)); "
...
def main():
    ...
    start = run_py("import live_news_dashboard as c" % () + chr(10) + "print(len(c.filter_rules()))")
    check("규칙은 지금 0개", start.stdout.strip() == "0", "규칙 %s개" % start.stdout.strip())

    made = run_py("import live_news_dashboard as c" + chr(10) +
                  "c.add_filter_rule('%s', enabled=True)" % MARK + chr(10) +
                  "print([r['pattern'] for r in c.filter_rules()])")
    check("규칙을 켤 수 있음", MARK in (made.stdout or ""), (made.stdout or made.stderr or "").strip()[:60])

    ins = run_py(
        "import live_news_dashboard as c" + chr(10) +
        "from datetime import datetime, timezone" + chr(10) +
        "now = datetime.now(timezone.utc).isoformat()" + chr(10) +
        "c.insert_articles([{'link': '%s', 'title': 'Probe %s headline'," % (LINK, MARK) + chr(10) +
        "  'summary': 'probe', 'source': 'Probe', 'source_type': 'rss', 'region': '글로벌'," + chr(10) +
        "  'category': '시장·가격', 'categories': ['시장·가격'], 'priority': 1," + chr(10) +
        "  'published_at': now, 'collected_at': now}])" + chr(10) +
        "print('caught', len(c.caught_links(200)))")
    ...
    off = run_py("import live_news_dashboard as c" + chr(10) +
                 "rid = [r['id'] for r in c.filter_rules()][0]" + chr(10) +
                 "c.set_filter_rule(rid, enabled=False)" + chr(10) +
                 "print('caught', len(c.caught_links(200)))")
    ...
    # 4. 숨긴 기사에서 규칙 뽑기
    learn = run_py("import live_news_dashboard as c" + chr(10) +
                   "before = len(c.filter_rules())" + chr(10) +
                   "c.learn_filter_rules()" + chr(10) +
                   "new = [r for r in c.filter_rules() if r['origin'] == 'learned']" + chr(10) +
                   "print('learned', len(new), '|', ', '.join(r['pattern'] for r in new[:3])," + chr(10) +
                   "      '| 꺼짐', all(not r['enabled'] for r in new))")
